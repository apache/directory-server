...
public class LdapNameTest extends TestCase 
{
        ...
        throws Exception
    {
        ...
        Name l_name2 = 
            new LdapName( "cn=HomeDir,cn=John,ou=Marketing,ou=East" ) ;
        Name l_name3 = 
            new LdapName( "cn=HomeDir,cn=John,ou=Marketing,ou=West" ) ;
        Name l_name4 = 
            new LdapName( "cn=Website,cn=John,ou=Marketing,ou=West" ) ;
        Name l_name5 = 
            new LdapName( "cn=Airline,cn=John,ou=Marketing,ou=West" ) ;
        ...
        throws Exception 
    {
        ...
        Name l_name3 = 
            m_parser.parse( "cn=HomeDir,cn=John,ou=Marketing,ou=West" ) ;
        Name l_name4 = 
            m_parser.parse( "cn=Website,cn=John,ou=Marketing,ou=West" ) ;
        Name l_name5 = 
            m_parser.parse( "cn=Airline,cn=John,ou=Marketing,ou=West" ) ;
     ...
     * @throws Exception anything goes wrong
     */
    /* Temporarely commented, as the getPrefix returns the wrong value.
    public void testGetPrefixModified()
        throws Exception
    {
        Name l_name = 
            m_parser.parse( "cn=HomeDir,cn=John,ou=Marketing,ou=East" ) ;
        
        Name prefix = l_name.getPrefix( 1 );
        
        assertEquals( "ou=East", prefix );
    }
    */
        ...
        throws Exception 
    {
        ...
        Name l_name6 = 
            m_parser.parse( "cn=HomeDir" ) ;
        Name l_name7 = 
            m_parser.parse( "cn=HomeDir,cn=John" ) ;
        Name l_name8 = 
            m_parser.parse( "cn=HomeDir,cn=John,ou=Marketing" ) ;
        ...
        throws Exception 
    {
        ...
        l_env.put( Context.SECURITY_AUTHENTICATION, "simple" ) ;
        l_env.put( Context.SECURITY_PRINCIPAL, "cn=admin,dc=example,dc=com" ) ;
        l_env.put( Context.SECURITY_CREDENTIALS, "jPasswordField1" ) ;
        l_env.put( Context.INITIAL_CONTEXT_FACTORY, 
            "com.sun.jndi.ldap.LdapCtxFactory" ) ;
        l_env.put( Context.PROVIDER_URL, 
            "ldap://localhost:1396/dc=example,dc=com" ) ;
        DirContext l_ctx = new InitialDirContext( l_env ) ;
        NamingEnumeration l_enum = l_ctx.listBindings( "" ) ;
        Name l_name0 = 
            m_parser.parse( "ou=Special Users,dc=example,dc=com" ) ;
        Name l_name1 = 
            m_parser.parse( "dc=example,dc=com" ) ;
        Name l_name2 = 
            m_parser.parse( "dc=com" ) ;
        Name l_name3 = 
            m_parser.parse( "ou=Special Users" ) ;
        Name l_name4 = 
            m_parser.parse( "ou=Special Users,dc=example" ) ;
        Name l_name5 = 
            m_parser.parse( "" ) ;
        while ( l_enum.hasMore() )
        {
            Binding l_binding = ( Binding ) l_enum.next() ;
            DirContext l_dirCtx = ( DirContext ) l_binding.getObject() ;
            NameParser l_parser = l_dirCtx.getNameParser( "" ) ;
            Name l_namex = l_parser.parse( l_dirCtx.getNameInNamespace() ) ;
            // DirContext l_dirCtx = ( DirContext ) l_enum.next() ;
        }
        */
    }
        ...
        throws Exception 
    {
        Name l_name = new LdapName() ;
        Name l_name0 = 
            m_parser.parse( "cn=HomeDir,ou=Marketing,ou=East" ) ;
        Name l_name1 = 
            m_parser.parse( "cn=John" ) ;
        Name l_name2 = 
            m_parser.parse( "cn=HomeDir,cn=John,ou=Marketing,ou=East" ) ;
        ...
        throws Exception 
    {
        ...
        Name l_name5 = 
            m_parser.parse( "cn=HomeDir,cn=John,ou=Marketing,ou=East,o=LL "
                + "Bean Inc." ) ;
        l_name.add( 0, "o=LL Bean Inc." ) ;
        assertEquals( l_name5, l_name ) ;
        
        Name l_name6 = 
            m_parser.parse( "cn=HomeDir,cn=John,ou=Marketing,ou=East,c=US,o=LL "
                + "Bean Inc." ) ;
        l_name.add( 1, "c=US" ) ;
        assertEquals( l_name6, l_name ) ;
        
        Name l_name7 = 
            m_parser.parse( "cn=HomeDir,cn=John,ou=Advertising,ou=Marketing,"
                + "ou=East,c=US,o=LL "
                + "Bean Inc." ) ;
        ...
        throws Exception 
    {
        ...
        l_name.remove( 1 ) ;
        Name l_name1 = 
            m_parser.parse( "cn=HomeDir,ou=East" ) ;
    ...
    public void testEqualsObject()
        throws Exception 
    {
        assertTrue( m_parser.parse( "ou=People" ).equals( 
            m_parser.parse( "ou=People" ) ) ) ; 
        assertTrue( ! m_parser.parse( "ou=People,dc=example,dc=com" ).equals( 
            m_parser.parse( "ou=People" ) ) ) ; 
        assertTrue( ! m_parser.parse( "ou=people" ).equals( 
            m_parser.parse( "ou=People" ) ) ) ; 
