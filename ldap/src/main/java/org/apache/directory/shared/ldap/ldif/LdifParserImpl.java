    ...
    public LdifEntry parse( String an_ldif ) throws NamingException
    {
        ...
        try
        {
            while ( ( l_line = l_in.readLine() ) != null )
            {
                if ( l_line.equalsIgnoreCase( "-" ) )
                {
                    if ( l_prevAttrValue != null )
                    {
                        if ( l_entry.getModType().equalsIgnoreCase( "modify" ) )
                        {
                            if ( l_currentModOp == -1 )
                            {
                                throw new LdapNamingException( "A modification"
                                    + " type must be supplied for a change " + "type of modify", ResultCodeEnum.OTHER );
                            }
                        ...
                        else
                        {
                            ...
                            {
                                l_entry.addAttribute( l_attrName, base64decode( l_attrValue ) );
                ...
                {
                    throw new LdapNamingException( "Line " + l_lineCount + " [" + l_line
                        + "] does not correspond to an LDIF entry " + "attribute value pair.\n{" + an_ldif + "}",
                        ResultCodeEnum.OTHER );
                }
                ...
                /*
                 * We need to check to see if the current line is an attribute,
                 * of if it a conrtol function
                 */
                ...
                else if ( l_attrName.equalsIgnoreCase( "add" ) )
                {
                    if ( !l_entry.getModType().equalsIgnoreCase( "modify" ) )
                    {
                        throw new LdapNamingException( "Cannot use modification " + l_attrName + " identifier on "
                            + l_entry.getModType() + " change type", ResultCodeEnum.OTHER );
                    }
                    ...
                    if ( l_entry.getModType().equalsIgnoreCase( "modify" ) )
                    {
                        if ( l_currentModOp == -1 )
                        {
                            throw new LdapNamingException( "A modification type must"
                                + " be supplied for a change type of modify", ResultCodeEnum.OTHER );
                        }
                        l_entry.addModificationItem( l_currentModOp, l_attrName, l_attrValue );
                    }
                    else
