...
/*
 * $Id: ProviderNexusAspect.java,v 1.5 2003/11/26 20:55:52 akarasulu Exp $
 *
 * -- (c) LDAPd Group                                                    --
...
public aspect ProviderNexusAspect
{
     ...
     * thread of execution.
     */
    private static ThreadLocal JndiProviderModule.s_contextStacks = 
        new ThreadLocal() ;
     ...
     * 
     * @param a_context the caller or the current context for the nexus call
     */
    private static void JndiProviderModule.push( LdapContext a_context )
    {
        Stack l_stack = ( Stack ) s_contextStacks.get() ;
        
        if ( null == l_stack )
        {
            l_stack = new Stack() ;
            s_contextStacks.set( l_stack ) ;
        }
        
        l_stack.push( a_context ) ;
    } 
     ...
     */
    private static LdapContext JndiProviderModule.pop()
    {
        Stack l_stack = ( Stack ) s_contextStacks.get() ;
        ...
        return ( LdapContext ) l_stack.pop() ;
    }
     ...
     */
    static LdapContext JndiProviderModule.peek()
    {
        Stack l_stack = ( Stack ) s_contextStacks.get() ;
        ...
        return ( LdapContext ) l_stack.peek() ;
    }
    ...
    {
        Stack l_stack = ( Stack ) JndiProviderModule.s_contextStacks.get() ;
     ...
     * 
     * @param a_caller the calling LdapContext.
     */
    pointcut jndiNexusCalls( Context a_caller ):
        this( a_caller ) &&
        ! this( AtomicBackend ) &&
        target( BackendNexus ) &&
        ! target( Database ) &&
        (
        call( public void add( Name, Attributes ) ) ||
        call( public void delete( Name ) ) ||
        call( public Name getMatchedDn( Name, boolean ) ) ||
        call( public Attributes getRootDSE() ) ||
        call( public Name getSuffix( boolean ) ) ||
        ...
        call( public NamingEnumeration 
           search( Name, Hashtable, ExprNode, Control [], SearchControls ) ) ) ;
    ...
    before( Context a_caller ):
        ...
        {
            JndiProviderModule.push( ( LdapContext ) a_caller ) ;
    ...
    after( Context a_caller ):
        jndiNexusCalls( a_caller ) 
        {
            LdapContext l_head = JndiProviderModule.pop() ;
