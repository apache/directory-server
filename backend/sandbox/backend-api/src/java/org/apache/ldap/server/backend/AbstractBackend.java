...
/*
 * $Id: AbstractBackend.java,v 1.9 2003/10/17 04:42:38 akarasulu Exp $
 *
 * -- (c) LDAPd Group                                                    --
...
import org.apache.ldap.server.backend.db.search.SearchResultEnumeration ;


/**
 * Abstract backend using a database and a search engine.  All the common code
 * between the SystemBackend and the DefaultBackend will be added to this super
 * class.
 *
...
{
    /**
     * the database used for this backend which is also initialized during
     * configuration time
     */
     ...
     */
    private SearchEngine m_engine = null ;
    ...
    /**
     * Gets the Database used by this AtomicBackend.
     *
    ...
    /**
     * Gets the DefaultSearchEngine used by this AtomicBackend to search the 
     * Database. 
     *
     ...
     *
     * @param a_database the database
     */
    protected void setDb( Database a_database )
    {
        m_db = a_database ;
    }
     ...
     *
     * @param a_engine the search engine
     */
    protected void setEngine( SearchEngine a_engine )
    {
        m_engine = a_engine ;
    }
    ...
    public void delete( Name a_dn ) throws NamingException
    {
        ...
        if ( m_db.getChildCount( l_id ) > 0 )
        {
            ContextNotEmptyException l_cnee = new ContextNotEmptyException(
                "[66] Cannot delete entry " + a_dn + " it has children!" ) ;
            l_cnee.setRemainingName( a_dn ) ;
            throw l_cnee ;
        }
    ...
    public void modify( Name a_dn, int a_modOp, Attributes a_mods )
        throws NamingException
    {
        m_db.modify( a_dn, a_modOp, a_mods ) ;
    }
    ...
    {
        return m_db.list( m_db.getEntryId( a_base.toString() ) ) ;
    }
    ...
    public NamingEnumeration search( Name a_base, Hashtable a_env, 
        ...
        throws NamingException
    {
        String [] l_attrIds = a_searchCtls.getReturningAttributes() ;
        NamingEnumeration l_underlying = null ;
        
        l_underlying = m_engine
            .search( a_base, a_env, a_filter, a_reqCtls, a_searchCtls ) ;
        
        return new SearchResultEnumeration( l_attrIds, l_underlying ) ;
    }
    ...
    {
        return m_db.lookup( m_db.getEntryId( a_dn.toString() ) ) ;
    }
    ...
    {
        return null != m_db.getEntryId( a_dn.toString() ) ;
    }
    ...
    public void modifyRdn( Name a_dn, String a_newRdn, boolean a_deleteOldRdn )
        throws NamingException
    {
        m_db.modifyRdn( a_dn, a_newRdn, a_deleteOldRdn ) ;
    }
    ...
    public void move( Name a_oldChildDn, Name a_newParentDn )
        throws NamingException
    {
        m_db.move( a_oldChildDn, a_newParentDn ) ;
    }
    ...
    {
        m_db.move( a_oldChildDn, a_newParentDn, a_newRdn, a_deleteOldRdn ) ;
    }
}