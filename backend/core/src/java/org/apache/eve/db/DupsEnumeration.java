...
/*
 * $Id: DupsEnumeration.java,v 1.4 2003/10/02 01:48:03 akarasulu Exp $
 *
 * -- (c) LDAPd Group                                                    --
    ...
    implements NamingEnumeration
{
    ...
    private final NoDupsEnumeration m_underlying ;
     ...
     * iterates over the TreeSet values in the m_duplicates Tuple. 
     */
    private Iterator m_dupIterator ;
     ...
     */
    DupsEnumeration( NoDupsEnumeration a_list )
        throws NamingException
    {
        m_underlying = a_list ;

        // Protect against closed cursors
        if ( ! m_underlying.hasMore() ) 
        {
            close() ;
            return ;
        }
    ...
    /**
     * Prefetches values into the m_prefetched Tuple taking into account that 
     * the returned Tuple values of the underlying enumeration m_list are really
     * TreeSets that hold multiple sorted values for the same key.  
     * 
     * <p> The values prefetched into the m_prefetched Tuple are actual values 
     * taken from the TreeSet.  So this NamingEnumeration simply expands out 
     * duplicate keyed Tuples which it returns.  m_iterator is an iteration over
     * the values held in the TreeSet returned by the underlying enumeration.  
     * The values pulled off of this iterator are put into m_prefetched. 
     * </p>
     *
     * @throws NamingException TODO
     */
    private void prefetch() throws NamingException
    {
         ...
         */
        while ( null == m_dupIterator || ! m_dupIterator.hasNext() ) 
        {
            /*
            ...
            if ( m_underlying.hasMore() ) 
            {
                m_duplicates = ( Tuple ) m_underlying.next() ;
                TreeSet l_set = ( TreeSet ) m_duplicates.getValue() ;

                if ( m_underlying.doAscendingScan() ) 
                {
                    m_dupIterator = l_set.iterator() ;
                } 
                else 
                {
                    /*
        ...
        /*
         * If we get to this point then m_iterator has more elements and 
         * m_duplicates holds the Tuple containing the key and TreeSet of 
         * values for that key which the m_iterator iterates over.  All we
        ...
        m_prefetched.setValue( m_dupIterator.next() ) ;
    }
}
