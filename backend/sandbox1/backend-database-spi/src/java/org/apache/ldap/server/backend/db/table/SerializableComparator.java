...
/*
 * $Id: SerializableComparator.java,v 1.4 2003/10/03 16:20:36 akarasulu Exp $
 *
 * -- (c) LDAPd Group                                                    --
...
public class SerializableComparator implements Comparator, Serializable
{
    ...
    private transient Comparator m_nonSerializable ;
     ...
     * 
     * @param a_comparator the comparator to associate
     */
    public static void set( Comparator a_comparator )
    {
        ...
        // Check first that a_comparator is not serializable
        if ( a_comparator instanceof Serializable )
        {
            throw new IllegalArgumentException( "Expecting non-Serializable " 
                + "argument Comparator but got Serializable Comparator." ) ;
        }
        
        s_comparators.set( a_comparator ) ;
        return ;    
    }
     ...
     * comparator.
     * 
     * @param a_nonSerializable the non Serializable Comparator wrapped
     */    
    public SerializableComparator( Comparator a_nonSerializable )
    {
        m_nonSerializable = a_nonSerializable ;
    }
     ...
     * @param a_nonSerializable the non Serializable Comparator wrapped
     */
    public void setNonSerializable( Comparator a_nonSerializable )
    {
        // Check first that a_comparator is not serializable
        if ( a_nonSerializable instanceof Serializable )
        {
            throw new IllegalArgumentException( "Expecting non-Serializable " 
                + "argument Comparator but got Serializable Comparator." ) ;
        }
    ...
    {
        return m_nonSerializable.compare( a_obj1, a_obj2 ) ;
    }
     ...
     * 
     * @param a_out the stream for serialization
     * @throws IOException if there are any failures in writing serialized obj
     */
    private void writeObject( ObjectOutputStream a_out )
        throws IOException
    {
     ...
     * 
     * @param a_in the stream to deserialize from
     ...
     */
    private void readObject( ObjectInputStream a_in )
        throws IOException, ClassNotFoundException
    {
        a_in.defaultReadObject() ;

        // Initialize
        m_nonSerializable = ( Comparator ) s_comparators.get() ;
        
        // Choke if we cannot find a Comparator 
        if ( null == m_nonSerializable ) 
        {
