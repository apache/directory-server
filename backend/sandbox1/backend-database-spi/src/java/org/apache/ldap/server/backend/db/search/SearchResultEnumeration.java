...
/*
 * $Id: SearchResultEnumeration.java,v 1.1 2003/10/17 03:54:55 akarasulu Exp $
 *
 * -- (c) LDAPd Group                                                    --
    ...
    implements DatabaseEnabled, NamingEnumeration
{
    ...
    private final String [] m_attrIds ;
     ...
     * 
     * @param a_attrIds the returned attributes
     * @param a_underlying the enumeration over IndexRecords
     * @throws NamingException if there is a problem accessing the underlying db
     */
    public SearchResultEnumeration( String [] a_attrIds, 
        NamingEnumeration a_underlying ) throws NamingException
    {
        m_attrIds = a_attrIds ;
    ...
    {
        IndexRecord l_rec = ( IndexRecord ) m_underlying.next() ;
        Attributes l_entry = new LockableAttributesImpl() ;
        String l_name = m_db.getEntryUpdn( l_rec.getEntryId() ) ;
        
        if ( null == l_rec.getAttributes() )
        {
            l_rec.setAttributes( m_db.lookup( l_rec.getEntryId() ) ) ;
        }
        
        for ( int ii = 0 ; ii < m_attrIds.length ; ii++ )
        {
            // there is no attribute by that name in the entry so we continue
            if ( null == l_entry.get( m_attrIds[ii] ) ) 
            {
                continue ;
            }
            
            // clone attribute to stuff into the new resultant entry
            Attribute l_attr = ( Attribute ) l_entry.clone() ;
            l_entry.put( l_attr ) ;
        }
        
        return new DbSearchResult( l_rec.getEntryId(), l_name, null, l_entry ) ;
    }
    ...
    {
        return m_underlying.hasMoreElements() ;
    }
