    ...
    public static final Object deserialize( byte[] buf )
    {
        ...
        while ( pos < buf.length )
        {
            ...
            if ( buf[pos] == ServerAttributeSerializerUtils.STRING_TYPE )
            {
                ...
                {
                    String value = ServerAttributeSerializerUtils.readString( buf, pos );
            ...
            else
            {
                ...
                while ( pos < buf.length && buf[pos] != SEPARATOR )
                {
                    byte[] value = ServerAttributeSerializerUtils.readBytes( buf, pos );
                    pos += value.length + 4;
                    attr.add( value );
                }
            }
    ...
    public static final byte[] serialize( Object attrsObj ) throws IOException
    {
        ...
        try
        {
            for ( NamingEnumeration<? extends Attribute> ii = attrs.getAll(); ii.hasMore(); )
            {
                ...
                pos = ServerAttributeSerializerUtils.write( buf, attr.getID(), pos );
                ...
                if ( first instanceof String )
                {
                    ...
                    for ( NamingEnumeration<?> jj = attr.getAll(); jj.hasMore(); )
                    {
                        ...
                        pos = ServerAttributeSerializerUtils.write( buf, value, pos );
                    }
                }
                else
                {
        ...
        try
        {
            for ( NamingEnumeration<? extends Attribute> ii = attrs.getAll(); ii.hasMore(); )
            {
                ...
                if ( ii.hasMore() )
                {
                    ...
                    size += ServerAttributeSerializerUtils.calculateSize( attr ) + 1;
                }
                else
                {
